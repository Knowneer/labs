...
int main() {
    ...
    //  отмечаем отличников (condition — указатель на функцию)
    filterStudents(arr, is_excellent);
    std::cout << "\nПосле filterStudents(is_excellent):\n";
    print_students(arr);


    // вес равномерный
    for (int i = 0; i < arr->size; ++i) {
        float avg = calculateAverage(&arr->students[i], weight_equal);
        std::cout << "AVG(" << arr->students[i].name << ") = " << avg << "\n";
    }

    //  Лямбда без захвата — можно как указатель на функцию:
    // вес: чем дальше индекс, тем больше вес (1,2,3,...) — без захвата!
    float (*w_linear)(int) = +[](int idx) -> float { return 1.0f + idx; };
    for (int i = 0; i < arr->size; ++i) {
        float avg = calculateAverage(&arr->students[i], w_linear);
        std::cout << "Weighted AVG(" << arr->students[i].name << ") = " << avg << "\n";
    }
